    ...
    with da.getVecArray(local_u) as un:
        for i in range(xs, xe):
            ...
            if i == 0 or i == n - 1:
                ...
                col.index = (i,)
                P.setValueStencil(row, col, 1.0/dt + (2*ui - uim1)/dx)
...
def update(frame):
    ...
    ax.set_title(f"Burgers (SNES + AIR) | t = {frame*dt:.3f}")
    return line,

ani = FuncAnimation(fig, update, frames=nt, interval=30, blit=True)
ani.save("burgers_snes_air.gif", dpi=150)
plt.close()

print("\nAnimation saved as burgers_snes_air.gif")
