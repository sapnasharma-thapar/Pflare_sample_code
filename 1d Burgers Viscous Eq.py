...
line, = ax.plot(x_axis, solution_history[0], lw=2, color='orange')
...
def update(frame):
    ...
    ax.set_title(f"Viscous Burgers (nu={nu}) | t = {frame*dt:.3f}")
    return line,

ani = FuncAnimation(fig, update, frames=nt, interval=30, blit=True)
ani.save("viscous_burgers_snes_air.gif", dpi=150)
plt.close()

print("\nAnimation saved as viscous_burgers_snes_air.gif")
